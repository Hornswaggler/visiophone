<template>
  ...
  >
    <div>
      <Header />
      <Navigation />
      off:{{ offSetX }}, {{ offSetY }} &nbsp;clientHeight:
      {{ clientHeight }} clientWidth: {{ clientWidth }} P1pos
      {{ playerOnePosition }} camera offset {{ cameraOffsetX }},
      {{ cameraOffsetY }}
    </div>
...
export default {
  ...
  computed: {
    ...
    spikes() {
      ...
      return this.entities.filter(
        (entity) => entity.type === ENTITY_TYPE.SPIKES
      );
    },
  ...
  async mounted() {
    ...
    await (async () => {
      ...
      return this.$store.dispatch("game/spawnEntity", {
        entity: PlayerModel({ x, y }),
      });
    })();
    ...
    for (let i = 0; i < MAX_SPIKES; i++) {
      ...
      await this.$store.dispatch("game/spawnEntity", {
        entity: SpikesModel({ x, y }),
      });
    }
  ...
  watch: {
    playerOnePosition({ x, y }, { x: oldX, y: oldY } = { x: 0, y: 0 }) {
      ...
      const playerScreenPositionX =
        x * this.tilesize * this.scaleFactor - this.offSetX;
      const playerScreenPositionY =
        y * this.tilesize * this.scaleFactor + this.offSetY;
      ...
      if (
        scalarX > 0 &&
        playerScreenPositionX >= this.clientWidth / 2 &&
      ...
      if (
        scalarY < 0 &&
        this.playerOnePosition.y * this.tilesize * this.scaleFactor +
          this.offSetY >
          this.clientHeight / 2 &&
  ...
  methods: {
    ...
    async resizeCanvas() {
      await this.$store.dispatch("game/resizeMap", {
    ...
    async drawMap({ dx, dy } = { dx: 0, dy: 0 }) {
      ...
      const targetStartX = Math.floor(
        0 - this.offSetX / (this.tilesize * this.scaleFactor)
      );
      ...
      const targetStartY = Math.floor(
        0 - this.offSetY / (this.tilesize * this.scaleFactor)
      );
      ...
      context.rect(
        ...
        0,
        this.width * this.tilesize + this.width * this.tilesize,
        this.height * this.tilesize
      );
