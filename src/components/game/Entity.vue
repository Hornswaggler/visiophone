<template>
  ...
  >
    ...
    <img
      ...
      :aria="entity.type"
      :style="{
        height: `${tilesize * scaleFactor}px`,
        width: `${tilesize * scaleFactor}px`,
      }"
    />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState("game", ["tilesize", "scaleFactor", "offSetX", "offSetY"]),
    ...
    path() {
      try {
        return require(`@/assets/${
          this.entity.path ? this.entity.path : defaultPath
        }`);
      } catch (e) {
    ...
    transform() {
      try {
        return `translate(${
          this.entity.x * (this.tilesize * this.scaleFactor) + this.offSetX
        }px, ${
          this.entity.y * (this.tilesize * this.scaleFactor) + this.offSetY
        }px)`;
      } catch (e) {
        console.error(e);
        return `translate(${this.entity.x * this.tilesize}px, ${
          this.entity.y * this.tilesize
        }px)`;
      }
    },
  },
