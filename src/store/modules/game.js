...
export default {
  ...
  actions: {
    async initMap({ commit, state: { width, height } }) {
      ...
      for (
        let i = 0;
        i < Object.keys(TILE_TYPE).length;
        i++, key = Object.keys(TILE_TYPE)[i]
      ) {
      ...
      const result = await (() =>
        new Promise((resolve) => {
          const callback = (x, y, value) => {
            mapData[x][y] = value
              ? cache[TILE_TYPE.FLOOR]
              : cache[TILE_TYPE.WALL];
          };
    ...
    resizeMap(
      { commit, state: { width, height, scaleFactor } },
    ...
    async handleInputOn({ state: { entities }, dispatch, commit }, { key }) {
      ...
      for (let i = 0; i < entities.length; i++) {
        ...
        activatedEntities.push({
          ...(await entity.handleInput(
            { dispatch, self: entity, commit },
            { key }
          )),
          entity: entity,
        });
    ...
    checkMapLocation(
      { state: { width, height, mapData, entities } },
      { x, y }
    ) {
      let result = {
        clip: false,
      ...
      try {
        const isWall =
          mapData[x] && mapData[x][y] != undefined
